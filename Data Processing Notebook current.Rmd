...
#******************************************************************************
#FUNCTION TO RANK CHARACTER COLUMNS
replace_values <- function(testData, look_up_table) {

#get column name for table join
column_name_for_key <- colnames(look_up_table[, 1])

#get column name for new ordinal ranking var
column_name_for_ordinal_var <- colnames(look_up_table[, 2])

#set key on that column
setkeyv(look_up_table, (column_name_for_key))

#set the key on the identical column in the original table
setkeyv(test_data, (column_name_for_key))

#match tables to create new ordinal var
test_data[look_up_table, (column_name_for_ordinal_var) := oldName_ordinal]
}
...
#use for general replacement of NAs
#setnafill(DT, "const", fill=0, cols = c("col1", "col2"))
...
# [FEATURE CREATION FOR CHAR VARS]{.underline}
...
### Char to ordinal column recoding

```{r}
# create replacement value tables
replacements1 = data.table(
  oldName = c("Buston", "Nu York", "Was DC"),
  oldName_ordinal = c(1, 2, 3))

replacements2 = data.table(
  oldName2 = c("A", "B", "C"),
  OldName2_ordinal = c(1, 2, 3))

replacements3 = data.table(
  oldName3 = c("aaa", "bbb", "ccc"),
  oldname3_ordinal = c(1, 2, 3))

#collect replacement tables in list
replacements_list <- list(replacements1, replacements2, replacements3)

#get number of objects in replacements_list
count_replacment_list <- length(replacements_list)

#loop through replacement table list
for (i in 1:length(replacements_list)) {
  #get consecutive replacement table
  look_up_table <- replacements_list[[i]]
  #run function and pass replacement table
  replace_values(test_data, look_up_table)
}
```
...
### Replace NAs for vars to be encoded

### Get all character and factor vars
