...
#DT <- rbindlist(rep(list(DT), 100000))
...
## Column Distribution Report

```{r paged.print=TRUE}
# Table with basic statistics
smbinning.eda(DT_sample_1, rounding = 3)$eda
gc()
```
...
## Impute NA for incorrect missing values
...
## Move bypass variable to new table
...
for (col in weighted_bin_cols) {
  ...
  set(DT, j = new_col_name, value = sum(DT[[col]] == 1) / nrow(DT))
}


rm(weighted_bin_cols, col_sums, DT_rows)
gc()
```
